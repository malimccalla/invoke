...
Why the `.work` format exists, why it is shaped the way it is, and what the previous attempts at this problem got wrong.

**None of this is normative.** The specification is [SPEC.md](SPEC.md). This document is the reasoning behind it, and it is written as working design notes rather than a finished argument.
...
**CWR (Common Works Registration)** is a CISAC-governed fixed-width flat file for bulk registration and revision of works between publishers, PROs and mechanical societies. It is real, it is the only game in town, and any serious implementation has to speak it fluently. But it is a **message**, not a **document**:
...
`.work` is a JSON document conforming to a published JSON Schema, with a filename convention. Nobody's permission is required. If it is ever worth registering `application/vnd.invoke.work+json` with IANA, that is a form submission in the vendor tree, not a standards battle.
...
The relational shape an implementation is likely to want. This is **not** the `.work` document structure — for that, see [SPEC §6](SPEC.md#6-object-model). It is modelled against CWR (v2.1 rev 8 / v2.2) so data can be delivered to societies without a translation layer bolted on at the end.
...
A globally unique creative or corporate identity. Keyed on **IPI Name Number** (11 digits) and **IPI Base Number** (`I-000000229-7`), with society affiliation held per right.
...
The earlier draft of this schema had real defects. They are listed here because finding them is the work, and because every one of them is now a numbered rule in [SPEC §8](SPEC.md#8-validation-rules).
...
**8 — Derivation was a JSON blob.** `isDerivative` / `isMedley` booleans plus an untyped `originalWorks` field. CWR needs structured `VER` (version of an original work) and `COM` (medley component) records. Model it as a self-referencing relation with a typed relation. The booleans are derivable from the relations; the relations are not derivable from the booleans.
...
Attach a **mandate** to each interested party: the rights they authorise to be granted on their behalf, the territories, the use classes, a rate card or floor, and exclusions (no political, no gambling, no tobacco, no AI training). A mandate is a standing offer, expressed as data.
